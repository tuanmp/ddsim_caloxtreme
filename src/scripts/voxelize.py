...
def digitize_in_z(shower_df: pd.DataFrame, voxel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    shower_df[
        "z_bin_index"
    ] = -1  # Initialize with -1 for pixels that don't fall into any bin

    for z_bin_index, z_bin_min, z_bin_max in z_bins.itertuples(index=False):
        in_bin = (shower_df["local_z"] >= z_bin_min) & (
            shower_df["local_z"] < z_bin_max
        )
...
def digitize_in_r_phi(shower_df: pd.DataFrame, voxel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for z_bin_index in voxel_df["z_bin_index"].unique():
        ...
        for _, row in layer.iterrows():
            ...
            local_phi = np.mod(
                shower_df["local_phi"], 2 * np.pi
            )  # Ensure phi is in the range [0, 2*pi]

            in_bin = (
                (shower_df["local_r"] >= r_bin_min)
                & (shower_df["local_r"] < r_bin_max)
                & (local_phi >= phi_bin_min)
                & (local_phi < phi_bin_max)
...
def digitize_shower(shower_df: pd.DataFrame, voxel_df: pd.DataFrame):
    ...
    energy = (
        shower_df.groupby(["z_bin_index", "r_bin_index", "phi_bin_index"])["energy"]
        .sum()
        .reset_index()
    )
    energy = energy.rename(columns={"energy": "binned_energy"})
    energy = energy[
        energy["z_bin_index"] != -1
    ]  # Filter out pixels that were not assigned to any z-bin
    energy = energy[
        energy["r_bin_index"] != -1
    ]  # Filter out pixels that were not assigned to any r-bin
    energy = energy[
        energy["phi_bin_index"] != -1
    ]  # Filter out pixels that were not assigned to any phi-bin
    # print(energy)

    energized_voxels = voxel_df.merge(
        energy, on=["z_bin_index", "r_bin_index", "phi_bin_index"], how="left"
    ).fillna(0)

    digitized_shower = shower_df.merge(
        energy, on=["z_bin_index", "r_bin_index", "phi_bin_index"], how="inner"
    ).dropna()
...
def read_binning_structure(xml_file):
    ...
    for layer in root.findall(".//Layer"):
        ...
        binning_structure.append(
            {"id": layer_id, "r_edges": r_edges, "n_bin_alpha": n_bin_alpha}
        )
...
def get_voxels(
    particle_df: pd.DataFrame, envelope_xml: str, binning_xml: str
) -> pd.DataFrame:
    """
    ...
    assert len(particle_df) == 1, (
        f"Expected exactly one particle per event for voxelization. Got {len(particle_df)} particles. {particle_df}"
    )
    particle_direction = (
        particle_df[
            [
    ...
    p_hat = vector.obj(
        x=particle_direction[0], y=particle_direction[1], z=particle_direction[2]
    )
    theta = p_hat.theta
    ...
    z_bin_edges = np.linspace(
        ecal_b_rmin, ecal_b_rmax, len(binning_structure) + 1
    ) / np.sin(theta)
    # print(z_bin_edges)
    ...
    z_bin_edges = np.concatenate(
        ([z_bin_edges[0]], selected_upper_edges[:-1], [z_bin_edges[-1]])
    )
    z_bins = np.stack(
        [z_bin_edges[:-1], z_bin_edges[1:]], axis=1
    )  # shape (num_z_bins, 2)
    ...
    assert (len(z_bins)) > 0, (
        "No valid z bins found. Check the binning structure and ECal barrel dimensions."
    )
    assert len(z_bins) == sum(non_empty_bins), (
        "Number of z bins must match the number of layers in the binning structure."
    )
    ...
    for i, layer in enumerate(non_empty_bins):
        ...
        r_bins = np.stack(
            [r_bin_edges[:-1], r_bin_edges[1:]], axis=1
        )  # shape (num_r_bins, 2)
        r_bin_centre = 0.5 * (r_bins[:, 0] + r_bins[:, 1]).reshape(
            -1, 1
        )  # shape (num_r_bins,1)
        r_bin_index = np.arange(len(r_bin_centre))  # shape (num_r_bins,)
        # print(r_bin_index.reshape(-1,1), r_bin_centre.reshape(-1,1), r_bins)
        r_block = np.concatenate(
            [r_bin_index.reshape(-1, 1), r_bin_centre.reshape(-1, 1), r_bins], axis=1
        )  # shape (num_r_bins, 4)

        # create phi bin edges
        phi_bin_edges = np.linspace(0, 2 * np.pi, n_bin_alpha + 1)
        phi_bins = np.stack(
            [phi_bin_edges[:-1], phi_bin_edges[1:]], axis=1
        )  # shape (num_phi_bins, 2)
        phi_bin_centre = 0.5 * (
            phi_bins[:, 0] + phi_bins[:, 1]
        )  # shape (num_phi_bins,)
        phi_bin_index = np.arange(len(phi_bin_centre)).reshape(
            -1, 1
        )  # shape (num_phi_bins,1)
        phi_block = np.concatenate(
            [phi_bin_index, phi_bin_centre.reshape(-1, 1), phi_bins], axis=1
        )  # shape (num_phi_bins, 4)

        local_bin_centre = (
            np.array(
                np.meshgrid(z_bin_index, r_bin_index, phi_bin_index, indexing="ij")
            )
            .reshape(3, -1)
            .T
        )  # shape (num_bins, 3)
        ...
        bin_df = pd.DataFrame(
            local_bin_centre, columns=["z_bin_index", "r_bin_index", "phi_bin_index"]
        )
        ...
        r_df = pd.DataFrame(
            r_block, columns=["r_bin_index", "r_bin_centre", "r_bin_min", "r_bin_max"]
        )
        phi_df = pd.DataFrame(
            phi_block,
            columns=["phi_bin_index", "phi_bin_centre", "phi_bin_min", "phi_bin_max"],
        )
        bin_df = bin_df.merge(r_df, on="r_bin_index").merge(phi_df, on="phi_bin_index")

        bin_df["rphi_bin_centre_x"] = bin_df["r_bin_centre"] * np.cos(
            bin_df["phi_bin_centre"]
        )
        bin_df["rphi_bin_centre_y"] = bin_df["r_bin_centre"] * np.sin(
            bin_df["phi_bin_centre"]
        )

        bin_centre_local = bin_df[
            ["rphi_bin_centre_x", "rphi_bin_centre_y", "z_bin_centre"]
        ].to_numpy()  # shape (num_bins, 3)
        bin_centre_global = (RT @ bin_centre_local.T).T  # shape (num_bins, 3)
        bin_df[
            ["bin_centre_global_x", "bin_centre_global_y", "bin_centre_global_z"]
        ] = bin_centre_global
        bin_df["bin_centre_global_r"] = np.sqrt(
            bin_df["bin_centre_global_x"] ** 2 + bin_df["bin_centre_global_y"] ** 2
        )
