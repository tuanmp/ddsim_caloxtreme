...
def parse_args():
    ...
    parser.add_argument(
        "--threads", help="Number of threads to use", type=int, default=None
    )
...
def log_particle_gun_config(ddsim, logger):
    ...
    if ddsim.gun.energy is not None:
        ...
        logger.info(
            f"  Momentum range: {ddsim.gun.momentumMin} - {ddsim.gun.momentumMax}"
        )
...
def configure_physics(ddsim, config, logger):
    ...
    if hasattr(config, "truthParticleHandler"):
        ...
        logger.info(
            "Setting truth particle handler to default Geant4TCUserParticleHandler"
        )
...
def configure_verbosity_and_ui(ddsim, config, logger):
    ...
    logger.info(
        f"Set Python logging level to {py_logging.getLevelName(python_level)} for DDSim modules"
    )
...
def main():
    ...
    try:
        ...
        if not getattr(config, "single_particle", False):
            ...
            logger.info(
                f"Using input filename: {input_filename} (from {'command-line' if args.input_file else 'config' if hasattr(config, 'input_filename') else 'default'})"
            )
        ...
        with timer.record("DD4hep Simulation"):
            if num_processes == 1:
                ...
                logger.info(
                    f"Running in multi-process mode with {num_processes} processes"
                )
                # Create a list of configurations for each process with different seeds
                output_paths = [
                    output_dir / f"edm4hep_proc_{i}.root" for i in range(num_processes)
                ]
                arg_list = [
                    (input_path, output_paths[i], config, setup_logging(f"Prog {i}"))
