...
def run_command(
    ...
    if stream:
        if not merge_streams:
            ...
            logger.debug(
                "merge_streams=False requested with stream=True; forcing merge_streams=True to avoid deadlocks."
            )
            merge_streams = True
...
def detect_process_type_from_files(process_dir):
    ...
    if shower_card.exists():
        ...
        logger.warning(
            "Could not detect process type - neither shower_card.dat nor pythia8_card.dat found"
        )
...
def customize_cards_for_process_type(
    process_dir, config, process_type, script_name="ColliderML"
):
    """
    ...
    if (
        hasattr(config, "card_customizations")
        and "run_card" in config.card_customizations
    ):
    ...
    if process_type == "born":
        ...
        if (
            hasattr(config, "card_customizations")
            and "shower_card" in config.card_customizations
        ):
            logger.info("Customizing shower_card.dat for NLO process...")
            customize_card_with_regex(
                shower_card_path, config.card_customizations["shower_card"]
            )
        else:
            logger.info(
                "No shower_card customizations specified. Using MadGraph defaults."
            )
        ...
        if (
            hasattr(config, "card_customizations")
            and "pythia8_card" in config.card_customizations
        ):
            logger.info("Customizing pythia8_card.dat for loop-induced process...")
            customize_card_with_regex(
                pythia8_card_path, config.card_customizations["pythia8_card"]
            )
        else:
            logger.info(
                "No pythia8_card customizations specified. Using MadGraph defaults."
            )
