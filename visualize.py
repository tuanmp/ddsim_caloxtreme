...
binning_xml = (
    "/global/cfs/cdirs/m3443/usr/pmtuan/ddsim_caloxtreme/binning_dataset_1_pions.xml"
)

st.set_page_config(
    page_title="Calorimeter Visualization", layout="wide", page_icon="📈"
)
...
def make_voxel_3d_chart(voxels, particle_df, barrel_df, selected_z_layer):
    ...
    particle_direction = particle_df[
        [
    ...
    voxel_3d_figure.add_trace(voxel_3d_chart)
    voxel_3d_figure.add_trace(direction_line)
...
def make_3d_shower_chart(barrel_df, selected_z_layer=None):
    ...
    shower_chart = go.Scatter3d(
        ...
        mode="markers",
        marker=dict(
            size=barrel_df["energy"]
            / barrel_df["energy"].max()
            * 60,  # Scale marker size by energy
            colorscale="Plotly3",
...
if len(preprocessed_array) > 0:
    cols = st.columns(2)
    with cols[0]:
        event_number = st.number_input(
            "Select event number",
            min_value=0,
            max_value=len(preprocessed_array) - 1,
            value=0,
            step=1,
            format="%d",
        )
    with cols[1]:
        deltaR = st.number_input(
            "Select deltaR for hit selection",
            min_value=0.0,
            max_value=0.1,
            value=0.05,
            step=0.01,
    ...
    barrel_contrib_keys = [
        key
        for key in event_data.fields
        if key.startswith(f"{BARREL_CONTRIBUTION_KEY}.")
    ]
    ...
    print(
        ...
        barrel_contrib_df["ECalBarrelCollectionContributions.energy"].sum(),
    )
    ...
    mominetum_at_endpoint = particle_df[
        [
            "MCParticles_momentumAtEndpoint_x",
            "MCParticles_momentumAtEndpoint_y",
            "MCParticles_momentumAtEndpoint_z",
        ]
    ].to_numpy()
    print("particle momentum: ", np.linalg.norm(momentum, axis=1))
    print(
        "particle momentum at endpoint: ", np.linalg.norm(mominetum_at_endpoint, axis=1)
    )
    ...
    barrel_contrib_df = barrel_contrib_df.rename(
        columns=lambda col: col.replace(".", "_").split("_", 1)[-1]
    )

    barrel_contrib_df = barrel_contrib_df.astype({"PDG": int})

    truth_particle = particle_df.iloc[0]
    vertex = truth_particle[
        ["MCParticles_vertex_x", "MCParticles_vertex_y", "MCParticles_vertex_z"]
    ].to_numpy()
    direction = truth_particle[
        ["MCParticles_momentum_x", "MCParticles_momentum_y", "MCParticles_momentum_z"]
    ].to_numpy()
        ...
        if len(barrel_df) == 0:
            ...
            xy_barrel_fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=np.clip(
                            barrel_df["energy"] / barrel_df["energy"].max() * 15, 3, 20
                        ),
            ...
            xy_barrel_contrib_chart = (
                altair.Chart(barrel_contrib_df)
                .mark_circle()
                .encode(
                    x=altair.X(
                        "cell_x",
                        scale=altair.Scale(domain=[XY_LIMITS[0], XY_LIMITS[1]]),
                    ),
                    y=altair.Y(
                        "cell_y",
                        scale=altair.Scale(domain=[XY_LIMITS[0], XY_LIMITS[1]]),
                    ...
                    color=altair.Color("PDG", legend=None).scale(
                        scheme="category10",
                        domain=sorted(barrel_contrib_df["PDG"].unique()),
                    ),
                    tooltip=["energy"],
                )
                .properties(
                    title="Barrel Calorimeter Contributions",
                    width=PLOT_SIZE[0],
                    height=PLOT_SIZE[1],
                )
                .interactive()
            ...
            rz_direcion_chart = (
                altair.Chart(direction_rz_df)
                .mark_line(color="red", opacity=0.5)
                .encode(x="z", y="r")
                .properties(
                    title="Particle Direction", width=PLOT_SIZE[0], height=PLOT_SIZE[1]
                )
                .interactive()
            )

            rz_barrel_contrib_chart = (
                altair.Chart(barrel_contrib_df)
                .mark_circle()
                .encode(
                    x=altair.X(
                        "cell_z", scale=altair.Scale(domain=[Z_LIMIT[0], Z_LIMIT[1]])
                    ),
                    y=altair.Y(
                        "cell_r", scale=altair.Scale(domain=[R_LIMIT[0], R_LIMIT[1]])
                    ),
                    size="energy",
                    color=altair.Color("PDG").scale(
                        scheme="category10",
                        domain=sorted(barrel_contrib_df["PDG"].unique()),
                    ),
                    tooltip=["energy"],
                ...
                .properties(
                    title="Barrel Calorimeter Contributions (Z-R)",
                    width=PLOT_SIZE[0],
                    height=PLOT_SIZE[1],
                )
                .interactive()
            ...
            st.info(
                "The following charts show the barrel calorimeter showers as energy deposits on individual cells. The size of each point corresponds to the energy deposited on the cell."
            )
            with st.container():
                cols = st.columns(2)
                with cols[0]:
            ...
            with st.container():
                ...
                selected_z_layer = st.multiselect(
                    "Select Z layers to display",
                    options=all_z_layers,
                    default=all_z_layers,
                )
                show_voxels = st.checkbox(
                    "Show voxelized shower representation", value=True
                )
                ...
                with st.spinner("Rendering 3D voxel visualization...", show_time=True):
                    figure = go.Figure()
                    voxel_centers, direction_line, voxel_meshes = make_voxel_3d_chart(
                        voxels, particle_df, barrel_df, selected_z_layer
                    )
                    ...
                    if show_shower:
                        shower_chart = make_3d_shower_chart(
                            digitized_barrel_df, selected_z_layer
                        )
                    ...
                    st.plotly_chart(
                        figure, width="stretch", height="content", key="voxel_3d_figure"
                    )
