...
def plot_layer_comparison(
    hlf_class, data, reference_class, reference_data, arg, show=False
):
    ...
    for idx, layer_id in enumerate(reference_class.relevantLayers):
        ...
        reference_class._DrawSingleLayer(
            reference_data_processed,
            idx,
            filename=None,
...
def plot_Etot_Einc_discrete(hlf_class, reference_class, arg, ratio=False):
    ...
    for i in range(len(target_energies) - 1):
        ...
        which_showers_ref = (
            (reference_class.Einc.squeeze() >= target_energies[i] - eps)
            & (reference_class.Einc.squeeze() < target_energies[i] + eps)
        ).squeeze()
        which_showers_hlf = (
            (hlf_class.Einc.squeeze() >= target_energies[i] - eps)
            & (hlf_class.Einc.squeeze() < target_energies[i] + eps)
        ).squeeze()
        ax = plt.subplot(4, 4, i + 1)
        gen = hlf_class.GetEtot()[which_showers_hlf]
        ref = reference_class.GetEtot()[which_showers_ref]
        counts_ref, _, _ = ax.hist(
            reference_class.GetEtot()[which_showers_ref]
        ...
        if arg.mode in ["all", "hist-chi", "hist"]:
            ...
            with open(
                os.path.join(
                    arg.output_dir, "histogram_chi2_{}.txt".format(arg.dataset)
                ),
                "a",
            ) as f:
    ...
    ax.axis("off")
    filename = os.path.join(
        arg.output_dir, "Etot_Einc_dataset_{}_E_i.{}".format(arg.dataset, plt_ext)
    )
    plt.savefig(filename, dpi=300)
    plt.close()
...
def plot_Etot_Einc(hlf_class, reference_class, arg, ratio=False):
    ...
    counts_data, _, _ = ax0.hist(
        hlf_class.GetEtot() / hlf_class.Einc.squeeze(),
        bins=bins,
        color="blue",
        label=label1,
        histtype="step",
    ...
    if ratio:
        eps = 1e-8
        h_ratio = 100.0 * (counts_data - counts_ref) / (counts_ref + eps)
    ...
    if arg.mode in ["all", "hist-p", "hist"]:
        filename = os.path.join(
            arg.output_dir, "Etot_Einc_dataset_{}.{}".format(arg.dataset, plt_ext)
        )
        plt.savefig(filename, dpi=300)
    if arg.mode in ["all", "hist-chi", "hist"]:
        seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_E_layers(hlf_class, reference_class, arg, ratio=False):
    ...
    for key in hlf_class.GetElayers().keys():
        ...
        if arg.x_scale == "log":
            ...
            bins = 40
        counts_ref, bins, _ = ax0.hist(
            reference_class.GetElayers()[key],
            bins=bins,
            facecolor="silver",
            label=label2,
            density=True,
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "E_layer_{}_dataset_{}.{}".format(key, arg.dataset, plt_ext),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECEtas(hlf_class, reference_class, arg, ratio=False):
    ...
    for key in hlf_class.GetECEtas().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "ECEta_layer_{}_dataset_{}.{}".format(key, arg.dataset, plt_ext),
            )
            plt.savefig(filename, dpi=300)
...
def plot_ECPhis(hlf_class, reference_class, arg, ratio=False):
    ...
    for key in hlf_class.GetECPhis().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "ECPhi_layer_{}_dataset_{}.{}".format(key, arg.dataset, plt_ext),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECWidthEtas(hlf_class, reference_class, arg, ratio=False):
    ...
    for key in hlf_class.GetWidthEtas().keys():
        ...
        bins = np.linspace(*lim, 101)
        counts_ref, _, _ = ax0.hist(
            reference_class.GetWidthEtas()[key],
            bins=bins,
            facecolor="silver",
            label=label2,
            density=True,
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "WidthEta_layer_{}_dataset_{}.{}".format(key, arg.dataset, plt_ext),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
            print(
                "Separation power of Width Eta layer {} histogram: {}".format(key, seps)
            )
            with open(
                os.path.join(
                    arg.output_dir, "histogram_chi2_{}.txt".format(arg.dataset)
...
def plot_ECWidthPhis(hlf_class, reference_class, arg, ratio=False):
    ...
    for key in hlf_class.GetWidthPhis().keys():
        ...
        bins = np.linspace(*lim, 101)
        counts_ref, _, _ = ax0.hist(
            reference_class.GetWidthPhis()[key],
            bins=bins,
            facecolor="silver",
            label=label2,
            density=True,
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "WidthPhi_layer_{}_dataset_{}.{}".format(key, arg.dataset, plt_ext),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
            print(
                "Separation power of Width Phi layer {} histogram: {}".format(key, seps)
            )
            with open(
                os.path.join(
                    arg.output_dir, "histogram_chi2_{}.txt".format(arg.dataset)
...
def plot_cell_dist(shower_arr, ref_shower_arr, arg, ratio=True):
    ...
    if arg.x_scale == "log":
        bins = np.logspace(np.log10(arg.min_energy), np.log10(ref_shower_arr.max()), 50)
    else:
        bins = 50

    print(
        "voxel range",
        np.amax(ref_shower_arr.flatten()),
        np.amin(ref_shower_arr.flatten()),
    )

    eps = 1e-16
    counts_ref, _, _ = ax0.hist(
        ref_shower_arr.flatten(),
        bins=bins,
        facecolor="silver",
        label=label2,
        density=True,
    ...
    if arg.mode in ["all", "hist-p", "hist"]:
        filename = os.path.join(
            arg.output_dir, "voxel_energy_dataset_{}.{}".format(arg.dataset, plt_ext)
        )
        plt.savefig(filename, dpi=300)
    if arg.mode in ["all", "hist-chi", "hist"]:
        seps = _separation_power(counts_ref, counts_data, bins)
